...
- Bumped container versions
  - `0.46.0` - [impartsecurity/inspector](https://hub.docker.com/r/impartsecurity/inspector/tags)
  - `0.46.0` - [impartsecurity/control-node](https://hub.docker.com/r/impartsecurity/control-node/tags)
  - `0.46.0` - [impartsecurity/k8s-webhook-injector](https://hub.docker.com/r/impartsecurity/k8s-webhook-injector/tags)
  - `0.46.0` - [impartsecurity/k8s-sidecar-init](https://hub.docker.com/r/impartsecurity/k8s-sidecar-init/tags)

## [0.45.2] - 2026-06-29

### Added

- `inspector.clientHashSeed.autoGenerate` (default `true`) — the injector generates the client-hash seed in-cluster and copies it into each injected namespace, so the Attackers data pipeline works without manually creating a Secret per namespace. Grants the injector cluster-wide `secrets` `get`/`create`; set `false` to opt out, or set `clientHashSeedSecretRef` to manage the seed yourself.
- `inspector.clientHashSeed.secretName` (default `impart-client-hash-seed`) — name of the autogenerated `Secret`.
- `inspector.clientHashSeed.purgeOnUninstall` (default `false`) — seeds persist across `helm uninstall` by default; set `true` to install a release-scoped `pre-delete` hook that deletes this release's seeds.

### Changed

- `inspector.penaltyBoxDataEnabled` now defaults to `true` — penalty-box events/counts upload to the API by default (requires a client-hash seed).
- The injector webhook uses `sideEffects: NoneOnDryRun` when seed autogeneration is on (it writes a Secret on real admissions).
- Bumped container versions
  - `0.45.2` - [impartsecurity/inspector](https://hub.docker.com/r/impartsecurity/inspector/tags)
  - `0.45.2` - [impartsecurity/control-node](https://hub.docker.com/r/impartsecurity/control-node/tags)
  - `0.45.2` - [impartsecurity/k8s-webhook-injector](https://hub.docker.com/r/impartsecurity/k8s-webhook-injector/tags)
  - `0.45.2` - [impartsecurity/k8s-sidecar-init](https://hub.docker.com/r/impartsecurity/k8s-sidecar-init/tags)
